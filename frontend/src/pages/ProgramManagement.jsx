        ...
        {showForm && (
          <div className="card mb-8 sm:mb-12 overflow-hidden border-brand-border/40 shadow-[0_8px_30px_rgb(0,0,0,0.3)] slide-in-bottom">
            <div className="border-b border-brand-border/30 pb-4 sm:pb-5 mb-6 sm:mb-8">
              ...
              <p className="text-white/45 mt-1 font-light flex items-center gap-2 text-sm">
            ...
            <form onSubmit={handleCreateOrUpdate} className="space-y-5 sm:space-y-6">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-2.5 text-white/45">Program Name</label>
                <input
                  type="text"
                  required
                  value={formData.name}
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-2.5 text-white/45">Description (Optional)</label>
                <textarea
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-2.5 text-white/45">Target Date</label>
                <input
                  type="date"
                  required
                  value={formData.date}
        ...
        <div className="space-y-3 sm:space-y-3">
          {loading ? (
            <div className="animate-pulse space-y-3">
              {[1, 2, 3].map(i => <div key={i} className="h-24 bg-brand-card-bg rounded-[1.25rem] sm:rounded-[1.5rem] border border-brand-border/20"></div>)}
            </div>
          ) : programs.length === 0 ? (
            <div className="text-center py-16 sm:py-24 bg-brand-card-bg rounded-[1.5rem] sm:rounded-[2rem] border border-dashed border-brand-border/20 fade-in-up">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-brand-dark-bg/50 rounded-[1.25rem] sm:rounded-[1.5rem] flex items-center justify-center mx-auto mb-4 sm:mb-6 border border-brand-border/20">
                <Calendar className="text-white/25" size={28} strokeWidth={1.5} />
              </div>
              <h3 className="text-xl sm:text-2xl font-semibold tracking-tight text-white">No Programs Yet</h3>
              <p className="text-base sm:text-lg text-white/45 mt-2 mb-6 sm:mb-8 font-light max-w-sm mx-auto px-4">Create your first fundraising program to get started.</p>
            ...
            <div className="grid gap-3 sm:gap-4">
              {programs.map((program, idx) => (
                <div key={program._id} className={`card p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 hover:translate-y-0 group fade-in-up delay-${Math.min(idx + 1, 8)}`}>
                  <div>
                    <div className="flex items-center gap-2 sm:gap-3 mb-2 flex-wrap">
                      <h3 className="text-lg sm:text-xl font-semibold tracking-tight">{program.name}</h3>
                      <span className={`text-[9px] sm:text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full font-semibold flex items-center gap-1.5 ${
                        program.status === 'Active' 
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' 
                        : 'bg-white/5 text-white/35 border border-white/10'
                      }`}>
                        {program.status === 'Active' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 pulse-dot"></span>}
                        {program.status}
                      </span>
                    </div>
                    <p className="text-white/45 mb-2 sm:mb-3 font-light leading-relaxed max-w-xl text-sm">{program.description || 'No description provided.'}</p>
                    <p className="text-[10px] sm:text-xs text-white/35 font-medium flex items-center gap-1.5 uppercase tracking-wider">
                  ...
                  {!isReadOnly && (
                    <div className="flex items-center gap-2 shrink-0">
                      <button 
                        ...
                        className="py-2.5 px-3 sm:px-4 text-sm flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl border border-brand-border/30 text-white/60 hover:text-white hover:bg-white/5 transition-all flex-1 sm:flex-none active:scale-[0.95]"
                      >
                      ...
                      <button 
                        ...
                        className="py-2.5 px-3 text-sm flex items-center justify-center gap-1.5 rounded-xl border border-red-500/20 text-red-400 hover:text-white hover:bg-red-500/20 transition-all active:scale-[0.95]"
                      >
