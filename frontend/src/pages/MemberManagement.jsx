...
const MemberManagement = () => {
  ...
  return (
    <div className="min-h-screen pb-24 bg-brand-grey">
      <header className="bg-brand-card-bg/70 backdrop-blur-xl border-b border-brand-border/50 sticky top-0 z-50 px-4 sm:px-6 h-14 sm:h-[4.5rem] flex items-center justify-between">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-12">
        {showForm && (
          <div className="card mb-8 sm:mb-12 overflow-hidden transform transition-all border-brand-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.3)] slide-in-bottom">
            <div className="border-b border-brand-border/50 pb-4 sm:pb-6 mb-6 sm:mb-8">
        ...
        <div className="space-y-3 sm:space-y-4">
          {loading ? (
            <div className="animate-pulse space-y-3 sm:space-y-4">
              {[1, 2, 3].map(i => <div key={i} className="h-20 bg-brand-card-bg rounded-[1.25rem] sm:rounded-[1.5rem] border border-brand-border/40"></div>)}
            </div>
          ) : members.length === 0 ? (
            <div className="text-center py-14 sm:py-20 bg-brand-card-bg rounded-[1.5rem] sm:rounded-[2rem] border border-dashed border-brand-border/30">
               <h3 className="text-lg sm:text-xl font-medium tracking-tight mt-4 text-white/80">No Members Found</h3>
               <p className="text-sm text-white/50 mt-1">Start by adding your executives.</p>
            </div>
          ) : (
            <div className="grid gap-3 sm:gap-4">
              {members.map(member => (
                <div key={member._id} className={`card p-4 sm:p-5 flex flex-col gap-3 sm:gap-0 sm:flex-row sm:items-center justify-between transition-all duration-300 ${!member.isActive ? 'opacity-60 bg-brand-card-bg/50' : ''}`}>
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-brand-orange/10 text-brand-orange-dark flex items-center justify-center shrink-0">
                      <Users size={18} strokeWidth={1.5} />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1 flex-wrap">
                        <h3 className="text-base sm:text-lg font-semibold tracking-tight leading-none">{member.name}</h3>
                        <span className={`text-[9px] sm:text-[10px] uppercase tracking-wider px-1.5 sm:px-2 py-0.5 rounded flex items-center font-bold ${
                        ...
                        {!member.isActive && (
                          <span className="text-[9px] sm:text-[10px] uppercase tracking-wider px-1.5 sm:px-2 py-0.5 rounded font-bold bg-red-500/10 text-red-400 border border-red-500/20">
                             Inactive
                          </span>
                        )}
                      </div>
                      <p className="text-xs sm:text-sm tracking-tight text-white/60 truncate">{member.phone} {member.address ? `• ${member.address}` : ''}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 ml-auto sm:ml-0 shrink-0">
                    <button 
                      ...
                      className="p-2 sm:p-2.5 btn-secondary rounded-xl text-white/70 hover:text-white hover:bg-white/10 flex items-center justify-center transition-colors"
                      title="Edit Profile"
                    >
                    ...
                    <button 
                      ...
                      className={`p-2 sm:p-2.5 rounded-xl border flex items-center justify-center transition-colors ${member.isActive ? 'border-red-500/20 text-red-400 hover:bg-red-500/10' : 'border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10'}`}
                    ...
                    <button 
                      ...
                      className="p-2 sm:p-2.5 border border-red-500/20 rounded-xl text-red-400 hover:text-white hover:bg-red-500/40 flex items-center justify-center transition-colors"
                      title="Delete Member"
                    >
